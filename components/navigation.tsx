...
export function Navigation() {
  ...
  const navItems = [
    ...
    {
      title: "Comparativas Cantonales",
      items: [
        { title: "Índice de Desarrollo Humano", href: "/#idh" },
        { title: "Índice de Desarrollo Social", href: "/#ids" },
        { title: "Índice de Progreso Social", href: "/#ips" },
        { title: "Índice de Competitividad Cantonal", href: "/#icc" },
      ],
    },
    {
    ...
    >
      <div className="container flex h-16 items-center justify-between">
              ...
              <NavigationMenuItem key={item.title}>
                {item.items ? (
                  <>
                    ...
                    <NavigationMenuContent>
                      <ul className="grid w-[400px] gap-3 p-4 bg-[#332B2B]">
                        {item.items.map((subItem) => (
                          <li key={subItem.title}>
                            <NavigationMenuLink asChild>
                              <a
                                href={subItem.href}
                                className="block select-none space-y-1 rounded-md p-3 leading-none no-underline outline-none transition-colors text-[#F5F5F5] hover:bg-[#5C2E2E] hover:text-white focus:bg-[#5C2E2E] focus:text-white"
                              >
                  ...
                  <NavigationMenuLink asChild>
                    <Link
                      href={item.href}
                      className="group inline-flex h-9 w-max items-center justify-center rounded-md bg-transparent px-4 py-2 text-sm font-medium transition-colors text-[#F5F5F5] hover:bg-[#5C2E2E] hover:text-white focus:bg-[#5C2E2E] focus:text-white focus:outline-none disabled:pointer-events-none disabled:opacity-50"
                    >
                      {item.title}
                    </Link>
                  </NavigationMenuLink>
        ...
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          ...
          <SheetContent side="right" className="w-[300px] sm:w-[400px] bg-[#2D2525]/85 border border-white/10 p-0 backdrop-blur-md">
            <div className="flex flex-col h-full">
              <div className="border-b border-white/10 px-6 py-6">
                <SheetTitle className="text-white text-lg font-bold">Menú</SheetTitle>
              </div>
                  ...
                  <div key={item.title}>
                    {item.items ? (
                      <div className="space-y-0">
                        ...
                        >
                          {item.title}
                          <ChevronDown className={`h-5 w-5 transition-transform duration-300 ${expandedItem === item.title ? 'rotate-180' : ''}`} />
                        </button>
                          ...
                          <div className="bg-[#2A2222] rounded-lg mx-2 my-1 space-y-0 animate-in fade-in-0 slide-in-from-top-2">
                            {item.items.map((subItem, idx) => (
                              <a
                                key={subItem.title}
                                href={subItem.href}
                                className={`block px-5 py-3 text-sm text-[#E5E5E5] hover:bg-[#5C2E2E] hover:text-white transition-colors ${idx === 0 ? 'rounded-t-lg' : ''} ${idx === item.items.length - 1 ? 'rounded-b-lg' : ''}`}
